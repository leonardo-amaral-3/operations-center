...
`OC_THEME` é a exceção, e deixou de ser a única forma de trocar de combinação: **o botão está no
cabeçalho do kanban**, e a escolha é lembrada entre uma abertura e a seguinte. O que restou à
variável é vencer o que ficou lembrado — abrir numa combinação sem mexer na preferência gravada. Não
é porta de teste como as outras, mas quem a exporta está testando, não usando.
